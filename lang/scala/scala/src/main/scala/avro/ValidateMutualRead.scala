  ...
  /**
    ...
    */
    ...
    def canRead(
      ...
      var error = false
      try error = Symbol.hasErrors(new ResolvingGrammarGenerator().generate(writtenWith, readUsing))
      catch {
        case e: IOException =>
          throw new SchemaValidationException(readUsing, writtenWith, e)
      }
      if (error) throw new SchemaValidationException(readUsing, writtenWith)
    }
}

class ValidateMutualRead extends SchemaValidationStrategy {
  /**
